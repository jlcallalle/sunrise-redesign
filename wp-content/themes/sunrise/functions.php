...
//Theme Options
//include_once '/theme-options/options-init.php';

//Add custom type 'banners'
require_once (TEMPLATEPATH . '/functions/custom-type-banners.php');

//Add support image featured
add_theme_support('post-thumbnails');

//no muestra version de wordpress
remove_action('wp_head', 'wp_generator');

//no mostrar error de autentificacion
...
//Add custom type for sliders home
...
//WordPress: Remove brackets around Category Post counts
...
function create_post_type() {
	$posts[0]['nombre'] = 'Consejo Noticia';
	$posts[0]['slug'] = 'consejo-noticia';
	$posts[0]['menu_icon'] = 'dashicons-book-alt';
	$posts[0]['soporte'] = array( 'title', 'editor', 'author','revisions', 'thumbnail','page-attributes','custom-fields' );	

	foreach($posts as $i=>$post){
		$nombre = $post['nombre'];
		$slug = $post['slug'];
		$menu_icono = $post['menu_icon'];
		...
		$labels = array(
			...
			'not_found_in_trash' => 'No hay ninguna página', 
			'parent_item_colon' => '',
			'menu_name' => $nombre
		 );
		...
		$args = array(
			...
			'query_var' => true,
			'rewrite' => array( 'slug' => $slug ), //slug es el nombre identificable, tanto en la url como en la llamada a la consulta
			'capability_type' => 'page',
			'has_archive' => true,
			'menu_icon'	=> $menu_icono, 
			'hierarchical' => true,
			'menu_position' => true,
			'supports' => $soporte
		); 
		register_post_type( $slug, $args );
		register_taxonomy( 'categoria-'.$slug, $slug, 
			 				array( 'label' => __('Categoria '.$nombre), 'hierarchical' => true,'show_ui' => true,'rewrite' => array( 'hierarchical' => true ) ) 
			 			 );
	}
}
